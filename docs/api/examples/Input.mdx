...
A short example of how to use the event driven input system to move an object around with arrow keys. In this example any user connected can move the objecct. 
...
who can move the object and not receieve the rendered position from the server via [`VoxelRenderer.receiveTransform = false`](api/VoxelRenderer#bool-receiveTransform)
